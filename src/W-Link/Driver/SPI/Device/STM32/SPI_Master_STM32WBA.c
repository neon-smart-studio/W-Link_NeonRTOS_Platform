...
int SPI_Master_Get_Clock_Freq(hwSPI_Index index)
{
    switch (index)
    {
        ...
        case hwSPI_Index_0:
        {
            ...
            if (src == RCC_SPI1CLKSOURCE_PCLK2)
                return HAL_RCC_GetPCLK2Freq();
#endif

#if defined(RCC_SPI1CLKSOURCE_SYSCLK)
            if (src == RCC_SPI1CLKSOURCE_SYSCLK)
                return HAL_RCC_GetSysClockFreq();
#endif

#if defined(RCC_SPI1CLKSOURCE_HSI)
            if (src == RCC_SPI1CLKSOURCE_HSI)
                return HSI_VALUE;
#endif
#endif
            return HAL_RCC_GetPCLK2Freq();
        ...
        case hwSPI_Index_2:
        {
#if defined(__HAL_RCC_GET_SPI3_SOURCE)
            uint32_t src = __HAL_RCC_GET_SPI3_SOURCE();

#if defined(RCC_SPI3CLKSOURCE_PCLK1)
            if (src == RCC_SPI3CLKSOURCE_PCLK1)
                return HAL_RCC_GetPCLK1Freq();
#endif

#if defined(RCC_SPI3CLKSOURCE_SYSCLK)
            if (src == RCC_SPI3CLKSOURCE_SYSCLK)
                return HAL_RCC_GetSysClockFreq();
#endif

#if defined(RCC_SPI3CLKSOURCE_HSI)
            if (src == RCC_SPI3CLKSOURCE_HSI)
                return HSI_VALUE;
#endif
#endif
            return HAL_RCC_GetPCLK1Freq();
...
)
{
    ...
    g_spi[index].Init.NSSPolarity                = SPI_NSS_POLARITY_LOW;
    g_spi[index].Init.FifoThreshold              = SPI_FIFO_THRESHOLD_01DATA;
    g_spi[index].Init.TxCRCInitializationPattern = SPI_CRC_INITIALIZATION_ALL_ZERO_PATTERN;
    g_spi[index].Init.RxCRCInitializationPattern = SPI_CRC_INITIALIZATION_ALL_ZERO_PATTERN;
    g_spi[index].Init.MasterSSIdleness           = SPI_MASTER_SS_IDLENESS_00CYCLE;
    g_spi[index].Init.MasterInterDataIdleness    = SPI_MASTER_INTERDATA_IDLENESS_00CYCLE;
    g_spi[index].Init.MasterReceiverAutoSusp     = SPI_MASTER_RX_AUTOSUSP_DISABLE;
    g_spi[index].Init.MasterKeepIOState          = SPI_MASTER_KEEP_IO_STATE_ENABLE;
    g_spi[index].Init.IOSwap                     = SPI_IO_SWAP_DISABLE;
