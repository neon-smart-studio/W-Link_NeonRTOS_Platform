...
#include "NFC/RFal/RFal_NFCA.h"
#include "NFC/RFal/RFal_T2T.h"
...
#define NDef_T2T_isT2TDevice(device) ((((device)->type == RFAL_NFC_LISTEN_TYPE_NFCA) && ((device)->dev.nfca.type == RFAL_NFCA_T2T)))
#define NDef_T2T_InvalidateCache(ctx) { (ctx)->subCtx.t2t.cacheAddr = 0xFFFFFFFFU; }

#define NDef_T2T_IsReadOnlyAccessGranted(ctx)  (((ctx)->cc.t2t.readAccess == 0x0U) && ((ctx)->cc.t2t.writeAccess == NDEF_T2T_WR_ACCESS_NONE))
#define NDef_T2T_IsReadWriteAccessGranted(ctx) (((ctx)->cc.t2t.readAccess == 0x0U) && ((ctx)->cc.t2t.writeAccess == NDEF_T2T_WR_ACCESS_GRANTED))

#define NDef_T2T_IsTransmissionError(err)      ( ((err) == NFC_FramingError) || ((err) == NFC_CRC_Error) || ((err) == NFC_ParityError) )

#define NDef_T2T_LogD(...)                                                                                  /*!< Macro for the debug log method                  */

static NFC_OpResult NDef_T2T_Poller_ReadBlock(NDef_Context *ctx, uint16_t blockAddr, uint8_t *buf);

static NFC_OpResult NDef_T2T_Poller_WriteBlock(NDef_Context *ctx, uint16_t blockAddr, const uint8_t *buf);
...
static NFC_OpResult NDef_T2T_Poller_ReadBlock(NDef_Context *ctx, uint16_t blockAddr, uint8_t *buf)
{
  ...
  //NDef_T2T_LogD("NDef_T2T_Poller_ReadBlock 0x%2.2x\r\n", blockAddr);
  ...
  if (secNo != ctx->subCtx.t2t.currentSecNo) {
    ret = RFal_T2T_PollerSectorSelect(secNo);
    if (ret < NFC_OK) {
      return ret;
    }
  ...
  do {
    ret = RFal_T2T_PollerRead(blNo, buf, NDEF_T2T_READ_RESP_SIZE, &rcvdLen);
  } while ((retry-- != 0U) && NDef_T2T_IsTransmissionError(ret));

  if ((ret == NFC_OK) && (rcvdLen != NDEF_T2T_READ_RESP_SIZE)) {
...
/*******************************************************************************/
NFC_OpResult NDef_T2T_Poller_ReadBytes(NDef_Context *ctx, uint32_t offset, uint32_t len, uint8_t *buf, uint32_t *rcvdLen)
{
  NFC_OpResult           ret;
  uint8_t              le;
  ...
  //NDef_T2T_LogD("NDef_T2T_Poller_ReadBytes offset: %d, len %d\r\n", offset, len);
  ...
  if ((offset >= ctx->subCtx.t2t.cacheAddr) && (offset < (ctx->subCtx.t2t.cacheAddr + NDEF_T2T_READ_RESP_SIZE)) && ((offset + len) < (ctx->subCtx.t2t.cacheAddr + NDEF_T2T_READ_RESP_SIZE))) {
    ...
    (void)memcpy(lvBuf, &ctx->subCtx.t2t.cacheBuf[offset - ctx->subCtx.t2t.cacheAddr], len);
  } else {
    do {
      ...
      if ((byteNo != 0U) || (lvLen < NDEF_T2T_READ_RESP_SIZE)) {
        ret = NDef_T2T_Poller_ReadBlock(ctx, blockAddr, ctx->subCtx.t2t.cacheBuf);
        if (ret < NFC_OK) {
          NDef_T2T_InvalidateCache(ctx);
          return ret;
        }
        ...
        if (le > 0U) {
          (void)memcpy(lvBuf, &ctx->subCtx.t2t.cacheBuf[byteNo], le);
        }
      } else {
        ret = NDef_T2T_Poller_ReadBlock(ctx, blockAddr, lvBuf);
        if (ret < NFC_OK) {
          return ret;
        }
        if ((lvLen == le) && (numOfValidBlocks == (NDEF_T2T_READ_RESP_SIZE / NDEF_T2T_BLOCK_SIZE))) {
          /* cache the last read block */
          (void)memcpy(&ctx->subCtx.t2t.cacheBuf[0], lvBuf, NDEF_T2T_READ_RESP_SIZE);
          ctx->subCtx.t2t.cacheAddr = (uint32_t)blockAddr * NDEF_T2T_BLOCK_SIZE;
        }
      }
      lvBuf     = &lvBuf[le];
...
/*******************************************************************************/
static NFC_OpResult NDef_T2T_Poller_SplitIntoAvailableAreas(NDef_Context *ctx, uint32_t offset, uint32_t len, uint32_t *physOffset, uint32_t *maxLen)
{
  uint32_t updatedOffset;
  uint32_t updatedLen;
  uint32_t i;

  //NDef_T2T_LogD("NDef_T2T_Poller_SplitIntoAvailableAreas offset %d, len %d\r\n", offset, len);
  ...
  for (i = 0; i < ctx->subCtx.t2t.nbrRsvdAreas; i++) {
    //NDef_T2T_LogD("NDef_T2T_Poller_SplitIntoAvailableAreas area #%d, start: %d Len: %d\r\n", i, ctx->subCtx.t2t.rsvdAreaFirstByteAddr[i], ctx->subCtx.t2t.rsvdAreaSize[i]);
...
/*******************************************************************************/
static NFC_OpResult NDef_T2T_Poller_ReadBytesFromAvailableAreas(NDef_Context *ctx, uint32_t offset, uint32_t len, uint8_t *buf, uint32_t *rcvdLen)
{
  NFC_OpResult ret;
  uint32_t curOffset;
  ...
  while (remainingLen > 0U) {
    (void)NDef_T2T_Poller_SplitIntoAvailableAreas(ctx, curOffset, remainingLen, &curPhyOffset, &maxLen);
    //NDef_T2T_LogD("NDef_T2T_Poller_ReadBytesFromAvailableAreas CurOffset: %d, curPhyOffset:%d Len :%d , buf offset: %d\r\n", curOffset,  curPhyOffset, maxLen, len - remainingLen);
    ret = NDef_T2T_Poller_ReadBytes(ctx, curPhyOffset, maxLen, &buf[len - remainingLen], &curRcvdLen);
    if (ret < NFC_OK) {
      return ret;
    }
    if (rcvdLen != NULL) {
...
/*******************************************************************************/
static NFC_OpResult NDef_T2T_ReadLField(NDef_Context *ctx)
{
  NFC_OpResult           ret;
  uint32_t             offset;
  uint8_t              data[3];
  ...
  offset++;
  ret = NDef_T2T_Poller_ReadBytesFromAvailableAreas(ctx, offset, 1, data, NULL);
  if (ret < NFC_OK) {
    /* Conclude procedure */
    return ret;
  }
  ...
  if (lenTLV == NDEF_T2T_3_BYTES_TLV_LEN) {
    ret = NDef_T2T_Poller_ReadBytesFromAvailableAreas(ctx, offset, 2, data, NULL);
    if (ret < NFC_OK) {
      /* Conclude procedure */
      return ret;
    }
  ...
  if (ctx->messageLen == 0U) {
    if (!(NDef_T2T_IsReadWriteAccessGranted(ctx))) {
      /* Conclude procedure  */
    ...
    if ((NDef_T2T_IsReadWriteAccessGranted(ctx))) {
      ...
      if (!(NDef_T2T_IsReadOnlyAccessGranted(ctx))) {
        /* Conclude procedure  */
        return NFC_RequestError;
      }
...
/*******************************************************************************/
NFC_OpResult NDef_T2T_Poller_ContextInitialization(NDef_Context *ctx, const RFal_NFC_Device *dev)
{
  if ((ctx == NULL) || (dev == NULL) || !NDef_T2T_isT2TDevice(dev)) {
    return NFC_InvalidParameter;
  }
...
/*******************************************************************************/
NFC_OpResult NDef_T2T_Poller_NdefDetect(NDef_Context *ctx, NDef_Info *info)
{
  NFC_OpResult           ret;
  uint8_t              data[3];
  uint32_t             offset;
  ...
  ret = NDef_T2T_Poller_ReadBytes(ctx, NDEF_T2T_CC_OFFSET, NDEF_T2T_CC_LEN, ctx->ccBuf, NULL);
  if (ret < NFC_OK) {
    /* Conclude procedure */
    return ret;
  }
  ...
  while ((offset < (NDEF_T2T_AREA_OFFSET + ctx->areaLen))) {
    ...
    if (typeTLV == NDEF_T2T_TLV_LOCK_CTRL) {
      ...
      ret = NDef_T2T_Poller_ReadBytesFromAvailableAreas(ctx, offset, NDEF_T2T_LOCK_CTRL_LEN, data, NULL);
      if (ret < NFC_OK) {
        /* Conclude procedure */
        return ret;
      }
    ...
    if (typeTLV == NDEF_T2T_TLV_MEMORY_CTRL) {
      ...
      ret = NDef_T2T_Poller_ReadBytesFromAvailableAreas(ctx, offset, NDEF_T2T_MEM_CTRL_LEN, data, NULL);
      if (ret < NFC_OK) {
        /* Conclude procedure */
        return ret;
      }
...
/*******************************************************************************/
NFC_OpResult NDef_T2T_Poller_ReadRawMessage(NDef_Context *ctx, uint8_t *buf, uint32_t bufLen, uint32_t *rcvdLen, bool single)
{
  NFC_OpResult ret;
  ...
  ret = NDef_T2T_Poller_ReadBytesFromAvailableAreas(ctx, ctx->messageOffset, ctx->messageLen, buf, rcvdLen);
  if (ret < NFC_OK) {
    ctx->state = NDEF_STATE_INVALID;
  }
  return ret;
...
static NFC_OpResult NDef_T2T_Poller_WriteBlock(NDef_Context *ctx, uint16_t blockAddr, const uint8_t *buf)
{
  ...
  retry = NDEF_T2T_N_RETRY_ERROR;
  do {
    ret = RFal_T2T_PollerWrite(blNo, buf);
  } while ((retry-- != 0U) && NDef_T2T_IsTransmissionError(ret));
...
/*******************************************************************************/
NFC_OpResult NDef_T2T_Poller_WriteBytes(NDef_Context *ctx, uint32_t offset, const uint8_t *buf, uint32_t len, bool pad, bool writeTerminator)
{
  NFC_OpResult           ret;
  uint32_t             lvOffset = offset;
  uint32_t             lvLen    = len;
  ...
  do {
    ...
    if ((byteNo != 0U) || (lvLen < NDEF_T2T_BLOCK_SIZE)) {
      if ((byteNo != 0U) || !pad) {
        ret = NDef_T2T_Poller_ReadBlock(ctx, blockAddr, tempBuf);
        if (ret < NFC_OK) {
          return ret;
        }
      }
      if ((byteNo + lvLen) < NDEF_T2T_BLOCK_SIZE) {
        if (pad) {
          (void)memset(&tempBuf[byteNo + lvLen], 0x00, NDEF_T2T_BLOCK_SIZE - (byteNo + lvLen));
        }
      ...
      if (le > 0U) {
        (void)memcpy(&tempBuf[byteNo], lvBuf, le);
      }
      ret = NDef_T2T_Poller_WriteBlock(ctx, blockAddr, tempBuf);
      if (ret < NFC_OK) {
        return ret;
      }
    } else {
      ret = NDef_T2T_Poller_WriteBlock(ctx, blockAddr, lvBuf);
      if (ret < NFC_OK) {
        return ret;
      }
    }
  ...
  if (lvWriteTerminator) {
    blockAddr++;
    (void)memset(tempBuf, 0x00, NDEF_T2T_BLOCK_SIZE);
    tempBuf[0] = NDEF_T2T_TLV_TERMINATOR;
    (void)NDef_T2T_Poller_WriteBlock(ctx, blockAddr, tempBuf);
  }
...
/*******************************************************************************/
static NFC_OpResult NDef_T2T_Poller_WriteBytesToAvailableAreas(NDef_Context *ctx, uint32_t offset, const uint8_t *buf, uint32_t len, bool pad, bool writeTerminator)
{
  NFC_OpResult ret;
  uint32_t curOffset;
  uint32_t curPhyOffset;
  ...
  while (remainingLen > 0U) {
    ...
    if ((remainingLen == maxLen) &&  writeTerminator) { /* last part */
      (void)NDef_T2T_Poller_SplitIntoAvailableAreas(ctx, curOffset, remainingLen + 1U, &curPhyOffset, &maxLenTerm);
      if ((remainingLen + 1U) == maxLenTerm) { /* check enough room for terminator in that area */
        term = true;
        lvWriteTerminator = false;
      }
    }
    ret = NDef_T2T_Poller_WriteBytes(ctx, curPhyOffset, &buf[len - remainingLen], maxLen, (remainingLen == maxLen) && pad, term);
    //NDef_T2T_LogD("NDef_T2T_Poller_WriteBytesToAvailableAreas CurOffset: %d, curPhyOffset: %d, Len :%d , buf offset: %d\r\n", curOffset, curPhyOffset, maxLen, len - remainingLen);
    if (ret < NFC_OK) {
      return ret;
    }
    remainingLen -= maxLen;
  ...
  if (lvWriteTerminator) {
    (void)NDef_T2T_Poller_SplitIntoAvailableAreas(ctx, curOffset, 1U, &curPhyOffset, &maxLen);
    termBuf[0] = NDEF_T2T_TLV_TERMINATOR;
    (void)NDef_T2T_Poller_WriteBytes(ctx, curPhyOffset, termBuf, 1U, pad, false);
  }
  return NFC_OK;
}

/*******************************************************************************/
NFC_OpResult NDef_T2T_Poller_WriteRawMessageLen(NDef_Context *ctx, uint32_t rawMessageLen, bool writeTerminator)
{
  NFC_OpResult           ret;
  ...
  if (writeTerminator && (rawMessageLen != 0U) && ((ctx->messageOffset + rawMessageLen) < (ctx->areaLen + NDEF_T2T_AREA_OFFSET))) {
    ...
    dataIt++;
    ret = NDef_T2T_Poller_WriteBytesToAvailableAreas(ctx, ctx->messageOffset + rawMessageLen, buf, dataIt, true, false);
    if (ret < NFC_OK) {
      return ret;
    }
  }
  ...
  ret = NDef_T2T_Poller_WriteBytesToAvailableAreas(ctx, ctx->subCtx.t2t.offsetNdefTLV, buf, dataIt, writeTerminator && (rawMessageLen == 0U), false);
  return ret;
}

/*******************************************************************************/
NFC_OpResult NDef_T2T_Poller_WriteRawMessage(NDef_Context *ctx, const uint8_t *buf, uint32_t bufLen)
{
  NFC_OpResult ret;
  ...
  ret = NDef_T2T_Poller_CheckAvailableSpace(ctx, bufLen);
  if (ret < NFC_OK) {
    /* Conclude procedures */
    return NFC_InvalidParameter;
  }
  ...
  ret = NDef_T2T_Poller_BeginWriteMessage(ctx, bufLen);
  if (ret < NFC_OK) {
    ctx->state = NDEF_STATE_INVALID;
    /* Conclude procedure */
    return ret;
  ...
  if (bufLen != 0U) {
    ...
    ret = NDef_T2T_Poller_WriteBytesToAvailableAreas(ctx, ctx->messageOffset, buf, bufLen, true, NDef_T2T_Poller_CheckAvailableSpace(ctx, bufLen + 1U) == NFC_OK);
    if (ret < NFC_OK) {
      /* Conclude procedure */
      ctx->state = NDEF_STATE_INVALID;
      return ret;
    ...
    ret = NDef_T2T_Poller_EndWriteMessage(ctx, bufLen, false);
    if (ret < NFC_OK) {
      /* Conclude procedure */
      ctx->state = NDEF_STATE_INVALID;
      return ret;
...
/*******************************************************************************/
NFC_OpResult NDef_T2T_Poller_TagFormat(NDef_Context *ctx, const NDef_CapabilityContainer *cc, uint32_t options)
{
  NFC_OpResult           ret;
  uint8_t              dataIt;
  ...
  if ((ctx->ccBuf[NDEF_T2T_CC_0] == 0U) && (ctx->ccBuf[NDEF_T2T_CC_1] == 0U) && (ctx->ccBuf[NDEF_T2T_CC_2] == 0U) && (ctx->ccBuf[NDEF_T2T_CC_3] == 0U)) {
    ...
    ret = NDef_T2T_Poller_WriteBlock(ctx, NDEF_T2T_CC_OFFSET / NDEF_T2T_BLOCK_SIZE, ctx->ccBuf);
    if (ret < NFC_OK) {
      return ret;
    }
  }
   ...
   */
  ret = NDef_T2T_Poller_WriteBlock(ctx, NDEF_T2T_AREA_OFFSET / NDEF_T2T_BLOCK_SIZE, emptyNdef);

  return ret;
}

/*******************************************************************************/
NFC_OpResult NDef_T2T_Poller_CheckPresence(NDef_Context *ctx)
{
  NFC_OpResult ret;
  uint16_t   blockAddr;
...
/*******************************************************************************/
NFC_OpResult NDef_T2T_Poller_CheckAvailableSpace(const NDef_Context *ctx, uint32_t messageLen)
{
  uint32_t lLen;
...
/*******************************************************************************/
NFC_OpResult NDef_T2T_Poller_BeginWriteMessage(NDef_Context *ctx, uint32_t messageLen)
{
  NFC_OpResult ret;
  uint32_t   lLen;
  ...
  /* TS T2T v1.0 7.5.3.4: reset L_Field to 0 */
  ret = NDef_T2T_Poller_WriteRawMessageLen(ctx, 0U, true);
  if (ret < NFC_OK) {
    /* Conclude procedure */
    ctx->state = NDEF_STATE_INVALID;
    return ret;
...
/*******************************************************************************/
NFC_OpResult NDef_T2T_Poller_EndWriteMessage(NDef_Context *ctx, uint32_t messageLen, bool writeTerminator)
{
  NFC_OpResult ret;
  ...
  /* TS T2T v1.0 7.5.3.6 & 7.5.3.7: update L_Field and write Terminator TLV */
  ret = NDef_T2T_Poller_WriteRawMessageLen(ctx, messageLen, writeTerminator);
  if (ret < NFC_OK) {
    /* Conclude procedure */
    ctx->state = NDEF_STATE_INVALID;
    return ret;
...
/*******************************************************************************/
NFC_OpResult NDef_T2T_Poller_SetReadOnly(NDef_Context *ctx)
{
  NFC_OpResult ret;
  ...
  ret = NDef_T2T_Poller_WriteBytes(ctx, NDEF_T2T_STATLOCK_OFFSET, staticBits, sizeof(staticBits), false, false);
  if (ret < NFC_OK) {
    return ret;
  }
  ...
  if (ctx->areaLen > NDEF_T2T_STATIC_MEM_SIZE) {
    ...
    ret = NDef_T2T_Poller_WriteBytes(ctx, ctx->subCtx.t2t.dynLockFirstByteAddr, dynLockBits, i, false, false);
    if (ret < NFC_OK) {
      return ret;
    }
  }
