...
void SPI_NVIC_Init(hwSPI_Index index)
{
    switch (index)
    {
        ...
        case hwSPI_Index_1:
#endif
            HAL_NVIC_SetPriority(SPI2_3_IRQn, SPI_IRQ_NVIC_PRIORITY, SPI_IRQ_NVIC_SUB_PRIORITY);
...
#endif
#if defined(SPI2_BASE) && defined(SPI3_BASE)
            if(!Spi_Master_Init_Status[hwSPI_Index_1] && !Spi_Master_Init_Status[hwSPI_Index_1])
#endif
            {
