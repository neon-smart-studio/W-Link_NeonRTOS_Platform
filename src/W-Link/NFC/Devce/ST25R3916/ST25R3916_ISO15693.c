...
static ST25R3916_ISO15693_PhyConfig_t ST25R3916_ISO15693_PhyConfig; /*!< current phy configuration */

static NFC_OpResult ST25R3916_ISO15693_PhyVCDCode1Of4(const uint8_t data, uint8_t *outbuffer, uint16_t maxOutBufLen, uint16_t *outBufLen)
{
  uint8_t tmp;
...
static NFC_OpResult ST25R3916_ISO15693_PhyVCDCode1Of256(const uint8_t data, uint8_t *outbuffer, uint16_t maxOutBufLen, uint16_t *outBufLen)
{
  uint8_t tmp;
  NFC_OpResult err = NFC_OK;
  uint16_t a;
...
{
  static struct ST25R3916_ISO15693_StreamConfig stream_config = {                                       /* MISRA 8.9 */
  ...
  memcpy((uint8_t *)&ST25R3916_ISO15693_PhyConfig, (const uint8_t *)config, sizeof(ST25R3916_ISO15693_PhyConfig_t));
...
{
  memcpy(config, &ST25R3916_ISO15693_PhyConfig, sizeof(ST25R3916_ISO15693_PhyConfig_t));
                                                     ...
                                                     uint8_t *outbuf, uint16_t outBufSize, uint16_t *actOutBufSize)
{
  ...
  *actOutBufSize = 0;
  ...
  if (ISO15693_VCD_CODING_1_4 == ST25R3916_ISO15693_PhyConfig.coding) {
    sof = ISO15693_DAT_SOF_1_4;
    eof = ISO15693_DAT_EOF_1_4;
    txFunc = ST25R3916_ISO15693_PhyVCDCode1Of4;
    *subbit_total_length = (
                             (1U   /* SOF */
    ...
    txFunc = ST25R3916_ISO15693_PhyVCDCode1Of256;
    *subbit_total_length = (
                             (1U   /* SOF */
  ...
  while ((err == NFC_OK) && sendCrc && (*offset < (length + 2U))) {
    uint16_t filled_size;
    if (0U == crc) {
      crc = ST25R3916_CRC_CalculateCcitt((uint16_t)((picopassMode) ? 0xE012U : 0xFFFFU),          /* In PicoPass Mode a different Preset Value is used   */
                                                        ...
                                                        bool picopassMode)
{
  ...
  *outBufPos = 0;
  ...
  if ((inBuf[0] & 0x1fU) != 0x17U) {
    ISO_15693_DEBUG("0x%x\n", ST25R3916_ISO15693_PhyBitBuffer[0]);
  ...
  *bitsBeforeCol = bp;
  ...
  if (*outBufPos > 2U) {
    ...
    crc = ST25R3916_CRC_CalculateCcitt(((picopassMode) ? 0xE012U : 0xFFFFU), outBuf, *outBufPos - 2U);
    crc = (uint16_t)((picopassMode) ? crc : ~crc);
