...
#define RFAL_NFCDEP_RWT_INI_MAX          RFal_NFC_Dep_WT2RWT( RFAL_NFCDEP_WT_INI_MAX ) /*!< RWT Initiator maximum value   */
...
#define RFAL_NFCDEP_RWT_TRG_MAX          RFal_NFC_Dep_WT2RWT( RFAL_NFCDEP_WT_TRG_MAX ) /*!< RWT Initiator maximum value         */
...
#define RFal_NFC_Dep_WT2RWT( wt )         ( (uint32_t)1U << (((uint32_t)(wt) & RFAL_NFCDEP_WT_MASK) + 12U) )                 /*!< Converts WT value to RWT (1/fc)               */

/*! Returns the BRS value from the given bit rate */
#define RFal_NFC_Dep_Dx2BRS( br )         ( (((uint8_t)(br) & RFAL_NFCDEP_BRS_Dx_MASK) << RFAL_NFCDEP_BRS_DSI_POS) | ((uint8_t)(br) & RFAL_NFCDEP_BRS_Dx_MASK) )

#define RFal_NFC_Dep_BRS2DRI( brs )       (uint8_t)( (uint8_t)(brs) & RFAL_NFCDEP_BRS_Dx_MASK )                              /*!< Returns the DRI value from the given BRS byte */
#define RFal_NFC_Dep_BRS2DSI( brs )       (uint8_t)( ((uint8_t)(brs) >> RFAL_NFCDEP_BRS_DSI_POS) & RFAL_NFCDEP_BRS_Dx_MASK ) /*!< Returns the DSI value from the given BRS byte */

#define RFal_NFC_Dep_PP2LR( PPx )         ( ((uint8_t)(PPx) & RFAL_NFCDEP_PP_LR_MASK ) >> RFAL_NFCDEP_PP_LR_SHIFT)  /*!< Returns the LR value from the given PPx byte  */
...
#define RFal_NFC_Dep_SetNFCID( dst, src, len )   memset( (dst), 0x00, RFAL_NFCDEP_NFCID3_LEN ); \
...
typedef bool (* RFal_NFC_Dep_DeactCallback)(void);
...
typedef struct {
  ...
  RFal_NFC_Dep_Role      role;     /*!< Current NFCIP role                                      */
  RFal_NFC_Dep_CommMode  commMode; /*!< Current NFCIP communication mode                        */
...
typedef union { /*  PRQA S 0750 # MISRA 19.2 - Both members of the union will not be used concurrently , device is only initiatior or target a time. No problem can occur. */
  struct {
    RFal_NFC_Dep_AtrRes  ATR_RES;      /*!< ATR RES            (Initiator mode)  */
    uint8_t           ATR_RESLen;   /*!< ATR RES length     (Initiator mode)  */
  } Target;                           /*!< Target                               */
  struct {
    RFal_NFC_Dep_AtrReq  ATR_REQ;      /*!< ATR REQ            (Target mode)     */
    uint8_t           ATR_REQLen;   /*!< ATR REQ length     (Target mode)     */
  } Initiator;                        /*!< Initiator                            */
} RFal_NFC_Dep_Activation;
...
typedef struct {
  ...
  RFal_BitRate       DSI;        /*!< Bit Rate coding from Initiator  to Target  */
  RFal_BitRate       DRI;        /*!< Bit Rate coding from Target to Initiator   */
...
typedef struct {
  RFal_NFC_Dep_Activation    activation;        /*!< Activation Info               */
  RFal_NFC_Dep_Info          info;              /*!< NFC-DEP device Info           */
} RFal_NFC_Dep_Device;
 ...
 */
typedef struct {
  RFal_NFC_Dep_CommMode commMode;       /*!< Initiator in Active P2P or Passive P2P*/
 ...
 */
typedef struct {
  RFal_NFC_Dep_CommMode commMode;                       /*!< Target in Active P2P or Passive P2P   */
  ...
  uint8_t            operParam;                      /*!< NFC-DEP Operation Parameter           */
} RFal_NFC_Dep_TargetParam;


/*! Structure of parameters to be passed in for nfcDepStartIpduTransceive              */
typedef struct {
  RFal_NFC_Dep_BufFormat *txBuf;         /*!< Transmit Buffer struct reference          */
...
typedef struct {
  RFal_NFC_Dep_PduBufFormat   *txBuf;    /*!< Transmit Buffer struct reference         */
  uint16_t                 txBufLen;  /*!< Transmit Buffer INF field length in Bytes*/
  RFal_NFC_Dep_PduBufFormat   *rxBuf;    /*!< Receive Buffer struct reference in Bytes */
  uint16_t                 *rxLen;    /*!< Received INF data length in Bytes        */
  RFal_NFC_Dep_BufFormat      *tmpBuf;   /*!< Temp buffer for single PDUs (internal)   */
  uint32_t                 FWT;       /*!< FWT to be used (ignored in Listen Mode)  */
  uint32_t                 dFWT;      /*!< Delta FWT to be used                     */
  uint16_t                 FSx;       /*!< Other device Frame Size (FSD or FSC)     */
  uint8_t                  DID;       /*!< Device ID (RFAL_ISODEP_NO_DID if no DID) */
} RFal_NFC_Dep_PduTxRxParam;

/*! Struct that holds all NFCIP data */
typedef struct {
  RFal_NFC_Dep_Configs       cfg;               /*!< Holds the current configuration to be used    */

  RFal_NFC_Dep_State         state;             /*!< Current state of the NFCIP module             */
  ...
  RFal_NFC_Dep_Device        *nfcDepDev;        /*!< Pointer to NFC-DEP device info                */

  uint32_t                RTOXTimer;         /*!< Timer used for RTOX                           */
  RFal_NFC_Dep_DeactCallback isDeactivating;    /*!< Deactivating flag check callback              */
  ...
  RFal_NFC_Dep_PduTxRxParam  PDUParam;          /*!< PDU TxRx params                               */
