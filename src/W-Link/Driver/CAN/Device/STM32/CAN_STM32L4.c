...
{
    CAN_HAL_IRQHandler(hwCAN_Index_1);
}
...
static void CAN_EnableClock(hwCAN_Index index)
{
    switch (index)
    {
#if defined(CAN1_BASE)
        ...
        case hwCAN_Index_1:
            /*
             * bxCAN CAN2 depends on CAN1 shared filter/clock domain.
             */
            __HAL_RCC_CAN1_CLK_ENABLE();
...
static void CAN_DisableClock(hwCAN_Index index)
{
    switch (index)
    {
#if defined(CAN1_BASE)
        case hwCAN_Index_0:
#if defined(CAN2_BASE)
            if (g_can[hwCAN_Index_1].Instance == NULL)
#endif
            {
...
{
    if (index >= hwCAN_Index_MAX || bitrate == 0U)
...
hwCAN_OpResult CAN_Instance_DeInit(hwCAN_Index index)
{
    ...
    if (HAL_CAN_DeInit(&g_can[index]) != HAL_OK)
        return hwCAN_HwError;

    CAN_DisableClock(index);
    g_can[index].Instance = NULL;
...
hwCAN_OpResult CAN_ConfigFilter(hwCAN_Index index)
{
    ...
    filter.FilterBank = (index == hwCAN_Index_0) ? 0U : 14U;
...
hwCAN_OpResult CAN_StopHardware(hwCAN_Index index)
{
    ...
    if (HAL_CAN_Stop(&g_can[index]) != HAL_OK)
        return hwCAN_HwError;
...
#endif /* defined(CAN1_BASE) || defined(CAN2_BASE) */
