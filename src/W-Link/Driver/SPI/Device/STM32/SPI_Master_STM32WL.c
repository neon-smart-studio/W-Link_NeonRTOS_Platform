...
#if defined(SPI2_BASE)
        case hwSPI_Index_1:
#if defined(__HAL_RCC_GET_SPI2_SOURCE)
        {
            uint32_t src = __HAL_RCC_GET_SPI2_SOURCE();

#if defined(RCC_SPI2CLKSOURCE_PCLK1)
            if (src == RCC_SPI2CLKSOURCE_PCLK1)
                return HAL_RCC_GetPCLK1Freq();
#endif
#if defined(RCC_SPI2CLKSOURCE_SYSCLK)
            if (src == RCC_SPI2CLKSOURCE_SYSCLK)
                return HAL_RCC_GetSysClockFreq();
#endif
#if defined(RCC_SPI2CLKSOURCE_HSI)
            if (src == RCC_SPI2CLKSOURCE_HSI)
                return HSI_VALUE;
#endif
        }
#endif
