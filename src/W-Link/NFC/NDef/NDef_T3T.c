...
#define NDef_T3T_isT3TDevice(device) ((device)->type == RFAL_NFC_LISTEN_TYPE_NFCF)
#define NDef_T3T_IsWriteFlagON(writeFlag) ((writeFlag) == NDEF_T3T_WRITEFLAG_ON)

#define NDef_T3T_LogD(...)                                                                                  /*!< Macro for the debug log method                  */

static NFC_OpResult NDef_T3T_Poller_ReadBlocks(NDef_Context *ctx, uint16_t blockNum, uint8_t nbBlocks, uint8_t *rxBuf, uint16_t rxBufLen, uint16_t *rcvLen);
static NFC_OpResult NDef_T3T_Poller_ReadAttributeInformationBlock(NDef_Context *ctx);
static NFC_OpResult NDef_T3T_Poller_WriteBlocks(NDef_Context *ctx, uint16_t blockNum, uint8_t nbBlocks, const uint8_t *dataBlocks);
static NFC_OpResult NDef_T3T_Poller_WriteAttributeInformationBlock(NDef_Context *ctx);
...
static NFC_OpResult NDef_T3T_Poller_ReadBlocks(NDef_Context *ctx, uint16_t blockNum, uint8_t nbBlocks, uint8_t *rxBuf, uint16_t rxBufLen, uint16_t *rcvLen)
{
  ...
  RFal_NFCF_ServBlockListParam servBlock;
  RFal_NFCF_BlockListElem     *listBlocks;
  uint8_t                    index;
  uint16_t                   rcvdLen = 0U;
  RFal_NFCF_Serv               serviceCodeLst = 0x000BU; /* serviceCodeLst */
  ...
  ret = RFal_NFCF_PollerCheck(ctx->subCtx.t3t.NFCID2, &servBlock, ctx->subCtx.t3t.rxbuf, (uint16_t)sizeof(ctx->subCtx.t3t.rxbuf), &rcvdLen);
  if (ret < NFC_OK) {
    return ret;
  }
  ...
  if (requestedDataSize > 0U) {
    (void)memcpy(rxBuf, &ctx->subCtx.t3t.rxbuf[NDEF_T3T_CHECK_NB_BLOCKS_LEN], requestedDataSize);
    if (rcvLen != NULL) {
...
NFC_OpResult NDef_T3T_Poller_ReadBytes(NDef_Context *ctx, uint32_t offset, uint32_t len, uint8_t *buf, uint32_t *rcvdLen)
{
  ...
  NDef_T3T_LogD("NDef_T3T_Poller_ReadBytes offset: 0x%8.8x, Len %d\r\n", offset, len);
  NDef_T3T_LogD("NDef_T3T_Poller_ReadBytes currentLen: %d, startBlock %d\r\n", currentLen, startBlock);
  ...
  if (startOffset != 0U) {
    ...
    res = NDef_T3T_Poller_ReadBlocks(ctx, startBlock, 1U /* One block */, ctx->subCtx.t3t.rxbuf, blockLen, &nbRead);
    if (res != NFC_OK) {
      /* Check result */
      result = res;
    } else if (nbRead != NDEF_T3T_BLOCKLEN) {
      /* Check length */
      result = NFC_MemoryCorrupt;
    } else {
      ...
      if (nbRead > 0U) {
        (void)memcpy(buf, &ctx->subCtx.t3t.rxbuf[offset], (uint32_t)nbRead);
      }
  ...
  while ((currentLen >= (uint32_t)blockLen) && (result == NFC_OK)) {
    if (currentLen < ((uint32_t)blockLen * nbBlocks)) {
      /* Reduce the nb of blocks to read */
      nbBlocks = (uint16_t)(currentLen / blockLen);
    }
    res = NDef_T3T_Poller_ReadBlocks(ctx, startBlock, (uint8_t)nbBlocks, ctx->subCtx.t3t.rxbuf, blockLen * nbBlocks, &nbRead);
    if (res != NFC_OK) {
      /* Check result */
      return res;
    } else if (nbRead != (blockLen * nbBlocks)) {
      /* Check length */
      return NFC_MemoryCorrupt;
    } else {
      (void)memcpy(&buf[lvRcvLen], ctx->subCtx.t3t.rxbuf, (uint32_t)currentLen);
      lvRcvLen   += nbRead;
      currentLen -= nbRead;
      startBlock += nbBlocks;
      NDef_T3T_LogD("NDef_T3T_Poller_ReadBytes currentLen: %d, startBlock %d\r\n", currentLen, startBlock);
  ...
  if ((currentLen > 0U) && (result == NFC_OK)) {
    /* Unaligned read, need to use a tmp buffer */
    res = NDef_T3T_Poller_ReadBlocks(ctx, startBlock, 1U /* One block */, ctx->subCtx.t3t.rxbuf, blockLen, &nbRead);
    if (res != NFC_OK) {
      /* Check result */
...
static NFC_OpResult NDef_T3T_Poller_ReadAttributeInformationBlock(NDef_Context *ctx)
{
  ...
  rxbuf   = ctx->ccBuf;
  retcode = NDef_T3T_Poller_ReadBlocks(ctx, NDEF_T3T_ATTRIB_INFO_BLOCK_NB, 1U /* One block */, rxbuf, NDEF_T3T_BLOCK_SIZE, &rcvLen);
  if ((retcode != NFC_OK) && (rcvLen != NDEF_T3T_BLOCK_SIZE)) {
    return retcode;
  }
...
{
  if ((ctx == NULL) || (dev == NULL) || !NDef_T3T_isT3TDevice(dev)) {
    return NFC_InvalidParameter;
  }

  (void)memcpy(&ctx->device, dev, sizeof(ctx->device));

  memcpy(&ctx->subCtx.t3t.NFCID2, dev->dev.nfcf.sensfRes.NFCID2, sizeof(ctx->subCtx.t3t.NFCID2));
...
{
  NFC_OpResult        retcode;
  RFal_FeliCaPollRes pollRes[NDEF_T3T_MAX_DEVICE];
  ...
  retcode = RFal_NFCF_PollerPoll(RFAL_FELICA_1_SLOT, NDEF_T3T_SYSTEMCODE, (uint8_t)RFAL_FELICA_POLL_RC_NO_REQUEST, pollRes, &devCnt, &collisions);
  if (retcode != NFC_OK) {
  ...
  if (memcmp(&(pollRes[0U][NDEF_T3T_SENSFRES_NFCID2]), ctx->subCtx.t3t.NFCID2, RFAL_NFCF_NFCID2_LEN) != 0) {
    return NFC_RequestError; /* Wrong UID */
  }
  ...
  retcode = NDef_T3T_Poller_ReadAttributeInformationBlock(ctx);
  if (retcode != NFC_OK) {
    return retcode;
  }
...
NFC_OpResult NDef_T3T_Poller_ReadRawMessage(NDef_Context *ctx, uint8_t *buf, uint32_t bufLen, uint32_t *rcvdLen, bool single)
{
  ...
  NDef_T3T_LogD("NDef_T3T_Poller_ReadRawMessage messageLen: %d\r\n", ctx->messageLen);
  ...
  if (!single) {
    ctx->state = NDEF_STATE_INVALID;
    ret = NDef_T3T_Poller_ReadAttributeInformationBlock(ctx);
    if (ret < NFC_OK) {
      return ret;
    }
    ctx->messageLen = ctx->cc.t3t.Ln;
  ...
  if (NDef_T3T_IsWriteFlagON(ctx->cc.t3t.writeFlag)) {
  ...
  ret = NDef_T3T_Poller_ReadBytes(ctx, ctx->messageOffset, ctx->messageLen, buf, rcvdLen);
  if (ret < NFC_OK) {
    ctx->state = NDEF_STATE_INVALID;
  }
  return ret;
...
static NFC_OpResult NDef_T3T_Poller_WriteBlocks(NDef_Context *ctx, uint16_t blockNum, uint8_t nbBlocks, const uint8_t *dataBlocks)
{
  ...
  RFal_NFCF_BlockListElem     *listBlocks;
  uint8_t                    index;
  RFal_NFCF_Serv               serviceCodeLst = 0x0009U;
  ...
  ret = RFal_NFCF_PollerUpdate(ctx->subCtx.t3t.NFCID2, &servBlock, ctx->subCtx.t3t.txbuf, (uint16_t)sizeof(ctx->subCtx.t3t.txbuf), dataBlocks, ctx->subCtx.t3t.rxbuf, (uint16_t)sizeof(ctx->subCtx.t3t.rxbuf));
...
NFC_OpResult NDef_T3T_Poller_WriteBytes(NDef_Context *ctx, uint32_t offset, const uint8_t *buf, uint32_t len, bool pad, bool writeTerminator)
{
  ...
  if (startOffset != 0U) {
    ...
    res = NDef_T3T_Poller_ReadBlocks(ctx, startBlock, 1, tmpBuf, blockLen, &nbRead);
    if (res != NFC_OK) {
      /* Check result */
      return res;
    }
    ...
    (void)memcpy(&tmpBuf[startOffset], buf, nbWrite);
    if (pad) {
      startOffset += nbWrite;
      nbWrite =  NDEF_T3T_BLOCKLEN - startOffset;
      if (nbWrite > 0U) {
        (void)memset(&tmpBuf[startOffset], 0x00, nbWrite);
      }
    }
    res = NDef_T3T_Poller_WriteBlocks(ctx, startBlock, 1U /* One block */, tmpBuf);
    if (res != NFC_OK) {
      return res;
    }
  ...
  while (currentLen >= (uint32_t)blockLen) {
    ...
    res     = NDef_T3T_Poller_WriteBlocks(ctx, startBlock, (uint8_t) nbBlocks, &buf[txtLen]);
    if (res != NFC_OK) {
      return res;
    }
    txtLen     += nbWrite;
  ...
  if (currentLen > 0U) {
    /* Unaligned write, need to use a tmp buffer */
    if (pad) {
      (void)memset(tmpBuf, 0x00, NDEF_T3T_BLOCKLEN);
    } else {
      res = NDef_T3T_Poller_ReadBlocks(ctx, startBlock, 1U /* One block */, tmpBuf, blockLen, &nbRead);
      if (res != NFC_OK) {
        /* Check result */
        return res;
      }
    ...
    (void)memcpy(tmpBuf, &buf[txtLen], currentLen);
    res = NDef_T3T_Poller_WriteBlocks(ctx, startBlock, 1U /* One block */, tmpBuf);
    if (res != NFC_OK) {
      return res;
    }
...
static NFC_OpResult NDef_T3T_Poller_WriteAttributeInformationBlock(NDef_Context *ctx)
{
  ...
  ret = NDef_T3T_Poller_WriteBlocks(ctx, NDEF_T3T_ATTRIB_INFO_BLOCK_NB, 1U /* One block */, buf);
  return ret;
}
...
NFC_OpResult NDef_T3T_Poller_WriteRawMessage(NDef_Context *ctx, const uint8_t *buf, uint32_t bufLen)
{
  ...
  ret = NDef_T3T_Poller_CheckAvailableSpace(ctx, bufLen);
  if (ret < NFC_OK) {
    /* Conclude procedure */
    return NFC_InvalidParameter;
  }
  ...
  ret = NDef_T3T_Poller_BeginWriteMessage(ctx, bufLen);
  if (ret < NFC_OK) {
    ctx->state = NDEF_STATE_INVALID;
    /* Conclude procedure */
    return ret;
  ...
  if (bufLen != 0U) {
    ...
    ret = NDef_T3T_Poller_WriteBytes(ctx, ctx->messageOffset, buf, bufLen, true, false);
    if (ret < NFC_OK) {
      /* Conclude procedure */
      ctx->state = NDEF_STATE_INVALID;
      return ret;
  ...
  ret = NDef_T3T_Poller_EndWriteMessage(ctx, bufLen, false);
  if (ret < NFC_OK) {
    /* Conclude procedure */
    ctx->state = NDEF_STATE_INVALID;
    return ret;
...
{
  NFC_OpResult        res;
  RFal_FeliCaPollRes buffOut[NDEF_T3T_MAX_DEVICE];
  uint8_t           devCnt     = NDEF_T3T_MAX_DEVICE;
  uint8_t           collisions = 0U;
  ...
  if (cc == NULL) {
    ...
    res = NDef_T3T_Poller_ReadAttributeInformationBlock(ctx);  /* Read current cc */
    if (res != NFC_OK) {
      return res;
    }
  } else {
  ...
  res = RFal_NFCF_PollerPoll(RFAL_FELICA_1_SLOT, NDEF_T3T_SYSTEMCODE, (uint8_t)RFAL_FELICA_POLL_RC_NO_REQUEST, buffOut, &devCnt, &collisions);
  if (res != NFC_OK) {
    return res;
  }
  res = RFal_NFCF_PollerPoll(RFAL_FELICA_1_SLOT, NDEF_T3T_SYSTEMCODE, (uint8_t)RFAL_FELICA_POLL_RC_SYSTEM_CODE, buffOut, &devCnt, &collisions);
  if (res != NFC_OK) {
    return res;
  }
  ...
  res = NDef_T3T_Poller_WriteAttributeInformationBlock(ctx);
  return res;
}
...
NFC_OpResult NDef_T3T_Poller_CheckPresence(NDef_Context *ctx)
{
  ...
  retcode = NDef_T3T_Poller_ReadBlocks(ctx, 0U /* First block */, 1U /* One Block */, ctx->subCtx.t3t.rxbuf, NDEF_T3T_BLOCKLEN, &nbRead);
  return retcode;
}
...
NFC_OpResult NDef_T3T_Poller_BeginWriteMessage(NDef_Context *ctx, uint32_t messageLen)
{
  ...
  ret                   = NDef_T3T_Poller_WriteAttributeInformationBlock(ctx);
  if (ret < NFC_OK) {
    /* Conclude procedure */
    ctx->state = NDEF_STATE_INVALID;
    return ret;
...
NFC_OpResult NDef_T3T_Poller_WriteRawMessageLen(NDef_Context *ctx, uint32_t rawMessageLen, bool writeTerminator)
{
  ...
  return NDef_T3T_Poller_EndWriteMessage(ctx, rawMessageLen, false);
}
