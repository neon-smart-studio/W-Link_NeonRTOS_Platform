...
uint32_t SPI_Map_PinConfig(hwSPI_Index index, hwGPIO_Pin pin)
{
    switch (index) {
        case hwSPI_Index_0:
            ...
            if (pin == hwGPIO_Pin_A4) return GPIO_PA4_SSI0RX;
            if (pin == hwGPIO_Pin_A5) return GPIO_PA5_SSI0TX;
            if (pin == hwGPIO_Pin_A3) return GPIO_PA3_SSI0FSS;
            break;
        case hwSPI_Index_1:
            if (pin == hwGPIO_Pin_F2) return GPIO_PF2_SSI1CLK;
            if (pin == hwGPIO_Pin_F0) return GPIO_PF0_SSI1RX;
            if (pin == hwGPIO_Pin_F1) return GPIO_PF1_SSI1TX;
            if (pin == hwGPIO_Pin_F3) return GPIO_PF3_SSI1FSS;
            break;
        case hwSPI_Index_2:
            if (pin == hwGPIO_Pin_B4) return GPIO_PB4_SSI2CLK;
            if (pin == hwGPIO_Pin_B6) return GPIO_PB6_SSI2RX;
            if (pin == hwGPIO_Pin_B7) return GPIO_PB7_SSI2TX;
            if (pin == hwGPIO_Pin_B5) return GPIO_PB5_SSI2FSS;
            break;
        case hwSPI_Index_3:
            if (pin == hwGPIO_Pin_D0) return GPIO_PD0_SSI3CLK;
            if (pin == hwGPIO_Pin_D2) return GPIO_PD2_SSI3RX;
            if (pin == hwGPIO_Pin_D3) return GPIO_PD3_SSI3TX;
            if (pin == hwGPIO_Pin_D1) return GPIO_PD1_SSI3FSS;
            break;
