...
#ifndef RFAL_ISO_DEP_H
#define RFAL_ISO_DEP_H
...
typedef union {/*  PRQA S 0750 # MISRA 19.2 - Both members of the union will not be used concurrently, device is only of type A or B at a time. Thus no problem can occur. */
  ...
  union {
    struct {
      RFal_ISO_Dep_Ats        ATS;               /*!< ATS response            (Poller mode)    */
      uint8_t              ATSLen;            /*!< ATS response length     (Poller mode)    */
    } Listener;
    struct {
      RFal_ISO_Dep_Rats      RATS;               /*!< RATS request          (Listener mode)    */
    } Poller;
  } A;
  ...
  union {
    struct {
      RFal_ISO_Dep_AttribRes  ATTRIB_RES;        /*!< ATTRIB_RES              (Poller mode)    */
      uint8_t              ATTRIB_RESLen;     /*!< ATTRIB_RES length       (Poller mode)    */
    } Listener;
    struct {
      RFal_ISO_Dep_AttribCmd  ATTRIB;            /*!< ATTRIB request        (Listener mode)    */
      uint8_t              ATTRIBLen;         /*!< ATTRIB request length (Listener mode)    */
    } Poller;
  } B;
} RFal_ISO_Dep_Activation;
...
typedef struct {
  ...
  RFal_BitRate       DSI;             /*!< Bit Rate coding from Listener (PICC) to Poller (PCD) */
  RFal_BitRate       DRI;             /*!< Bit Rate coding from Poller (PCD) to Listener (PICC) */
...
typedef struct {
  RFal_ISO_Dep_Activation    activation; /*!< Activation Info                                      */
  RFal_ISO_Dep_Info          info;       /*!< ISO-DEP (ISO14443-4) device Info                     */
} RFal_ISO_Dep_Device;
...
} RFal_ISO_Dep_ApduBufFormat;


/*! Listen Activation Parameters Structure */
typedef struct {
  RFal_ISO_Dep_BufFormat  *rxBuf;                       /*!< Receive Buffer struct reference          */
  uint16_t             *rxLen;                       /*!< Received INF data length in Bytes        */
  bool                 *isRxChaining;                /*!< Received data is not complete            */
  RFal_ISO_Dep_Device     *isoDepDev;                   /*!< ISO-DEP device info                      */
} RFal_ISO_Dep_ListenActvParam;


/*! Structure of parameters used on ISO DEP Transceive */
typedef struct {
  RFal_ISO_Dep_BufFormat  *txBuf;                       /*!< Transmit Buffer struct reference         */
  uint16_t             txBufLen;                     /*!< Transmit Buffer INF field length in Bytes*/
  bool                 isTxChaining;                 /*!< Transmit data is not complete            */
  RFal_ISO_Dep_BufFormat  *rxBuf;                       /*!< Receive Buffer struct reference in Bytes */
  uint16_t             *rxLen;                       /*!< Received INF data length in Bytes        */
  bool                 *isRxChaining;                /*!< Received data is not complete            */
...
} RFal_ISO_Dep_TxRxParam;


/*! Structure of parameters used on ISO DEP APDU Transceive */
typedef struct {
  RFal_ISO_Dep_ApduBufFormat  *txBuf;                   /*!< Transmit Buffer struct reference         */
  uint16_t                 txBufLen;                 /*!< Transmit Buffer INF field length in Bytes*/
  RFal_ISO_Dep_ApduBufFormat  *rxBuf;                   /*!< Receive Buffer struct reference in Bytes */
  uint16_t                 *rxLen;                   /*!< Received INF data length in Bytes        */
  RFal_ISO_Dep_BufFormat      *tmpBuf;                  /*!< Temp buffer for Rx I-Blocks (internal)   */
  uint32_t                 FWT;                      /*!< FWT to be used (ignored in Listen Mode)  */
  uint32_t                 dFWT;                     /*!< Delta FWT to be used                     */
  uint16_t                 FSx;                      /*!< Other device Frame Size (FSD or FSC)     */
  uint16_t                 ourFSx;                   /*!< Our device Frame Size (FSD or FSC)       */
  uint8_t                  DID;                      /*!< Device ID (RFAL_ISODEP_NO_DID if no DID) */
} RFal_ISO_Dep_ApduTxRxParam;
...
} RFal_ISO_Dep_ControlMsg;

/*! Internal structure to be used in handling of S(PARAMETERS) only */
typedef struct {
  uint8_t               pcb;       /*!< PCB byte                      */
  RFal_ISO_Dep_SParameter  sParam;    /*!< S(PARAMETERS)                 */
} RFal_ISO_Dep_ControlMsgSParam;
...
typedef struct {
  RFal_ISO_Dep_State state;         /*!< ISO-DEP module state                      */
  RFal_ISO_Dep_Role  role;          /*!< Current ISO-DEP role                      */
  ...
  RFal_BitRate    txBR;          /*!< Current Tx Bit Rate                       */
  RFal_BitRate    rxBR;          /*!< Current Rx Bit Rate                       */
  ...
  RFal_ComplianceMode compMode;   /*!< Compliance mode                           */
  ...
  union {  /*  PRQA S 0750 # MISRA 19.2 - Members of the union will not be used concurrently, only one frame at a time */
    RFal_ISO_Dep_Rats       ratsReq;
    RFal_ISO_Dep_PpsReq     ppsReq;

    RFal_ISO_Dep_AttribCmd  attribReq;
  } actv;                                   /*!< Activation buffer              */

  uint8_t                  *rxLen8;         /*!< Receive length (8-bit)         */
  RFal_ISO_Dep_Device         *actvDev;        /*!< Activation Device Info         */
  RFal_ISO_Dep_ListenActvParam actvParam;      /*!< Listen Activation context      */


  RFal_ISO_Dep_ApduTxRxParam APDUParam;        /*!< APDU TxRx params               */
